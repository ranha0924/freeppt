...
- **`index.html`** 을 브라우저로 열면 바로 실행됩니다. (폰트·이미지가 모두 내장된 **자기완결형 단일 파일** — 오프라인·자체 호스팅 OK)
...
빌드 스크립트(`src/build.py`)는 사용된 글자만 서브셋한 woff2 폰트와 `src/assets/`의 스크린샷을 모두 data URI로 인라인 삽입합니다.
...
src/
  ...
  build.py            폰트 서브셋 · 이미지 인라인 → index.html
